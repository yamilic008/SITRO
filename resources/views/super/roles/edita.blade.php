 ...
 <div class="row clearfix">
                <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
                    <div class="card">
                        ...
                        <div class="body">
                              <form id="form_validation" method="POST" action="{{ route('roles.update',$dato['id'])}}">
                                 {{ csrf_field() }}{{method_field('PUT')}}
                                  <div class="form-group form-float">
                                      <div class="form-line">
                                          <input id="nombre" type="text" class="form-control" name="name" required value="{{$dato->name}}">
                                          <label id="nombre" class="form-label">Nombre</label>
                                      </div>
                                  </div>
                                  <div class="form-group form-float">
                                      <div class="form-line">
                                          <input type="text" class="form-control" name="guard" required value="{{$dato->guard_name}}" disabled="">
                                ...
                                <div class="form-group form-float">
                                  ...
                                  <div class="form-group">
                                    <label class="form-label">Seleciona permisos para este Rol: </label>
                                    @foreach($permisos as $per)
                                    @if($per->guard_name === $dato->guard_name)
                                        <input type="checkbox" id="checkbox{{$per->id}}" name="permisos[]" value="{{$per->name}}" 
                                        @foreach($dato->getPermissionNames() as $us)
                                            {{$per->name === $us ? 'checked' : ''}}
                                        @endforeach
                                        >
                                        
                                        
                                        <label for="checkbox{{$per->id}}">{{$per->name}}</label>
                                    @endif
                                    @endforeach
                                  </div>
