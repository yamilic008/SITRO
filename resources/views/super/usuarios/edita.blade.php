 ...
 <div class="row clearfix">
                <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
                    <div class="card">
                        ...
                        <div class="body">
                              <form id="form_validation" method="POST" action="{{ route('usuario.update',$dato['id'])}}">
                                 {{ csrf_field() }}{{method_field('PUT')}}
                                  ...
                                  <div class="form-group form-float">
                                      <div class="form-line">
                                          <input type="email" class="form-control" name="email" required value="{{$dato->email}}">
                                ...
                                <div class="form-group form-float">
                                  ...
                                  <div class="form-group">
                                    <label class="form-label">Seleciona un rol: </label> <br>
                                    @foreach($roles as $id=>$name)
                                      <input type="checkbox" id="checkbox{{$id}}" name="roles[]" value="{{$id}}" 
                                        @foreach($dato->Roles as $us)
                                          {{$us->id === $id ? 'checked' : ''}}
                                        @endforeach
                                      >
                                      <label for="checkbox{{$id}}">{{$name}}</label> <br>
                                    @endforeach
                                  </div>
